...
//const char* filename = "C:\\Study\\ПГНИУ2\\3 курс\\ВГиАКГ\\labTest\\ConsoleApplication1\\anime_girlss.png";
...
bool createShaderProgram()
{
    ...
    const GLchar fshF[] =
        ...
        "void main()"
        "{"
        "   vec3 mycolor = vec3(1.0f, 0.0f, 0.0f);"
...
{
    if (bezie_points.empty()) return;
    Point p(bezie_points[0].x, bezie_points[0].y);
    bool first = true;
    if (bezie_points.size() == 2) {
        GLfloat x = bezie_points[0].x;
        GLfloat y = bezie_points[0].y;
        GLfloat nx = bezie_points[1].x;
        GLfloat ny = bezie_points[1].y;
        glm::vec2 nxy = glm::normalize(glm::vec2(nx - x, ny - y));
        Normals.push_back(Point(-nxy.y, nxy.x));
        Normals.push_back(Point(-nxy.y, nxy.x));
        return;
    }
    for (int i = 1; i < bezie_points.size()-1; i++)
    {
        GLfloat x = bezie_points[i].x;
        GLfloat y = bezie_points[i].y;
        GLfloat nx = bezie_points[i+1].x;
        GLfloat ny = bezie_points[i+1].y;
        glm::vec2 nxy = glm::normalize(glm::vec2(nx - p.x, ny - p.y));
        Normals.push_back(Point( - nxy.y, nxy.x));
...
bool createModelFigure()
{
    ...
    GLint vertex_size = k * 6 * cout_base;
    ...
    for (int j = 0; j < cout_base; j++) {
        for (int f =0;f<bezie_points.size();f++)
        {
            Point p = bezie_points[f];
            vec4 v(p.x, p.y, 0.0, 1.0);
            GLfloat a = base_angle * j;
            v = rotate(mat4(1.0f), radians(a), vec3(0, 1, 0)) * v;
            Point norm = Normals[f];
            vec4 v2(norm.x, norm.y, -1.0f, 1.0f);
            //v2 = rotate(mat4(1.0f), radians(a), vec3(0, 1, 0)) * v2;

            vertex_arr[i] = v[0];
            vertex_arr[i + 1] = v[1];
            vertex_arr[i + 2] = v[2];
            vertex_arr[i + 3] = v2[0];// abs(rand() % 255) / 255.0;
            vertex_arr[i + 4] = v2[1];// abs(rand() % 255) / 255.0;
            vertex_arr[i + 5] = v2[2];// abs(rand() % 255) / 255.0;
